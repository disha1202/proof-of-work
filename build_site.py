...
<script>
  ...
  function renderProfile(){
    ...
    if (p.twitter)  links.push(["Twitter", "https://twitter.com/" + p.twitter]);
    if (p.email)    links.push(["Email", "mailto:" + p.email]);
    if (p.blog)     links.push(["Website", p.blog]);
    document.getElementById("socials").innerHTML = links.map(([t,h]) =>
      `<a class="social" href="${h}" target="_blank" rel="noopener">${t}</a>`).join("");
  }
...
BENTO_CSS = """
    ...
    .avatar { width: 96px; height: 96px; border-radius: var(--avatar-radius, 50%); object-fit: cover; border: 2px solid var(--line); background: var(--bg-2); flex-shrink: 0; }
    .pinfo { display: flex; flex-direction: column; gap: 5px; min-width: 0; }
    .eyebrow { font-size: 0.68rem; text-transform: uppercase; letter-spacing: 0.2em; color: var(--ink-faint); }
    .pname { font-family: var(--display, inherit); font-weight: 700; font-size: clamp(1.9rem, 5vw, 2.7rem); line-height: 1.02; margin: 0; color: var(--ink); }
    .pbio { margin: 3px 0 0; color: var(--ink-soft); font-size: 0.9rem; line-height: 1.45; }
    .pmeta { margin: 2px 0 0; color: var(--ink-faint); font-size: 0.82rem; }
    .socials { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 10px; }
    .social { font-size: 0.76rem; text-decoration: none; color: var(--ink-soft); border: 1px solid var(--line); border-radius: 999px; padding: 4px 13px; transition: all 0.15s; }
    ...
    @media (max-width: 760px) {
      ...
      .tile-profile, .tile-lang { grid-column: span 2; grid-row: auto; }
    }
